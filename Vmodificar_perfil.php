<?php
	...
	if(mysqli_num_rows($resultado)==0 || $usuario['email']==$correo){ /*la primera condicion verifica que no exista un mail igual ya registrado; la segunda que el mail registrado hasta ahora sea igual que el que está ingresando nuevo*/
		$sql= "UPDATE usuarios SET documento='$DNI', direccion='$direccion', apellido='$apellido', nombre='$nombre', fechanac='$nacimiento', premium='$premium', fechap='$fechap', costop='$costop', administrador='$admin' WHERE idusuario=$idusuario";
		mysqli_query($conec,$sql) or die('Error: ' . mysqli_error($conec));
		session_destroy();
?>
		<script type="text/javascript"> alert ("Modificacion exitosa"); window.location.href='index.php' </script>
<?php
	}
	else {
?>
		<script type="text/javascript"> alert ("Hubo un error al modificar el perfil"); window.location.href='index.php' </script>
<?php
	}
?>